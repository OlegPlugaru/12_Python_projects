...
Louis wants to see which of his friens can fly.
...
MAX_FLYING_WEIGHT: Final[int] = 15
...
def calc_weight(weight: float) -> float:
    ...
    Look how the function just transforms data, frm float to float
    """
    return (weight + 32) / 8
...
def flying_friends(friends: dict[str, float]) -> None:
    ...
    """
    for name, earth_weigh in friends.items():
        zortan_weight = calc_weight(earth_weigh)
