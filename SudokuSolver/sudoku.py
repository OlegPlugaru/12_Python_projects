def find_next_empty(puzzle):
    # finds the next row, col on the puzzle that's no filled yet --> rep with -1
    # return row, col tuple (or (None, None) if there is none)

    # keep in mind taht we are using 0-8 for our indices
    for r in range(9):
        for c in range(9): # range(9) is 0m 1, 2, ... 8
            if puzzle[r][c] == -1:
                return r, c
...
def is_valid(puzzle, guess, row, col):
    ...
    # if we get here, these cheks pass
    return True
...
def solve_sudoku(puzzle):
    ...
    # return whether a solutin exists
    # mutates puzzle to be the solution (if solutin exists)
    ...
    for guess in range(1, 10): # range(1, 10) is 1, 2, 3, ... 9
        ...
        if is_valid(puzzle, guess, row, col):
            ...
            # step 4: recursively cal our function
